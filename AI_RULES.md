...
- **AD** (`maintenance-ad`): friendly headers in `app/constants/ad_excel_import.py` as `AD_EXCEL_COLUMN_MAPPING` (`AD NUMBER`, `SUBJECT`, `INSPECTION INTERVAL`, `DATE OF EFFECTIVITY` or `DATE OF EFFECTIVITY OR COMPLIANCE DATE` — any case; snake_case aliases accepted). `aircraft_fk` comes from form `inject_fields`. Dates accept `6/5/2023`, `23-Jul-23`, Excel serials, and ISO values.
- **AD work orders** (`maintenance-ad-work-orders`): canonical headers in `app/constants/ad_work_order_excel_import.py` as `AD_WORK_ORDER_EXCEL_COLUMN_MAPPING`: `WO NUMBER`, `LAST DONE AFTT`, `LAST DONE TACH`, `LAST DONE DATE`, `NEXT DUE AFTT`, `NEXT DUE TACH`, `ATL REF` (any case; snake_case and legacy `Next Done`/`ACTT` aliases accepted). API/DB fields: `work_order_number`, `last_done_aftt`, `last_done_tach`, `last_done_date`, `next_due_aftt`, `next_due_tach`, `atl_ref`. `ad_monitoring_fk` comes from form `inject_fields` (`ad_monitoring_id` or `ad_monitoring_fk`). Dates accept `6/5/2023`, `23-Jul-23`, Excel serials, and ISO values.
